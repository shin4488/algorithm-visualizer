export function init(): void {
  ...
  function setPivot(board: Board, barsEl: HTMLElement, idx: number | null){ board.pivotIndex = idx; Array.from(barsEl.querySelectorAll('.bar')).forEach((c: Element, i)=> c.classList.toggle('pivot', i === idx)); }
  ...
  function tick(board: Board){
    ...
    if(board.stepIndex >= board.steps.length){
      if(isBubble){ if(Global.timerBubble!=null){ clearInterval(Global.timerBubble); Global.timerBubble = null; } }
      else{ if(Global.timerQuick!=null){ clearInterval(Global.timerQuick); Global.timerQuick = null; updateQuickOverlay({lo:null,hi:null,k:null}); } }
  ...
  function pause(){
    ...
    if(Global.timerBubble!=null){ clearInterval(Global.timerBubble); Global.timerBubble=null; }
    if(Global.timerQuick !=null){ clearInterval(Global.timerQuick ); Global.timerQuick =null; }
  ...
  function restoreBoard(board: Board, snap: BoardSnapshot): void{ Object.assign(board, { data: snap.data.slice(), ids: snap.ids.slice(), steps: snap.steps.slice(), stepIndex: snap.stepIndex, pivotIndex: snap.pivotIndex, finished: snap.finished }); }
  function simulate(arr: number[], steps: Step[]): number[]{ const a = arr.slice(); for(const s of steps){ if(s.t==='swap'){ const {i,j}=s; if(i>=0&&j>=0){ [a[i],a[j]]=[a[j],a[i]]; } } } return a; }
  function isSortedAsc(a: number[]): boolean{ for(let i=1;i<a.length;i++){ if(a[i-1]>a[i]) return false; } return true; }
