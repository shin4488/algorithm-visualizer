...
export function init(): void {
  ...
  function applyStep(board: Board, barsEl: HTMLElement, step: Step | undefined) {
    ...
    if (step.t === 'compare') {
      ...
      if (board.kind === 'quick' && board.pivotIndex != null) {
        ...
        if (i === board.pivotIndex) newPivotIndex = j;
        else if (j === board.pivotIndex) newPivotIndex = i;
  ...
  function snapshotBoard(board: Board): BoardSnapshot {
    return {
      kind: board.kind,
      data: board.data.slice(),
      ids: board.ids.slice(),
      steps: board.steps.slice(),
      stepIndex: board.stepIndex,
      pivotIndex: board.pivotIndex,
      finished: board.finished,
    };
  }
  function restoreBoard(board: Board, snap: BoardSnapshot): void {
    Object.assign(board, {
      data: snap.data.slice(),
      ids: snap.ids.slice(),
      steps: snap.steps.slice(),
      stepIndex: snap.stepIndex,
      pivotIndex: snap.pivotIndex,
  ...
  function runSanityTests() {
    ...
    const globalSnap = {
      speed: Global.speed,
      slider: parseFloat(el.speed.value),
      label: el.speedVal.textContent || '',
    };
    ...
    // T0: isSortedAsc
    {
      const asc = [1, 2, 3, 4, 5],
        dsc = [5, 4, 3, 2, 1];
      const okAsc = isSortedAsc(asc) === true,
        okDsc = isSortedAsc(dsc) === false;
      if (okAsc && okDsc) {
        ok('isSortedAsc basic');
      } else {
        ng('isSortedAsc basic', `${okAsc},${okDsc}`);
      }
    }
    // T1: genArray/permutation
    {
      ...
      if (arr.length === n && set.size === n && Math.min(...arr) === 1 && Math.max(...arr) === n) {
        ok('genArray/permutation');
      } else {
        ng('genArray/permutation', JSON.stringify(arr));
      }
    }
    // T2: bubble correctness
    {
      ...
      for (const c of cases) {
        ...
        if (!isSortedAsc(res)) {
          pass = false;
          ng('bubble', `${JSON.stringify(c)} -> ${JSON.stringify(res)}`);
          break;
        }
      }
      if (pass) ok('bubble');
    ...
    // T3: quick correctness
    {
      ...
      for (const c of cases) {
        ...
        if (!isSortedAsc(res)) {
          pass = false;
          ng('quick', `${JSON.stringify(c)} -> ${JSON.stringify(res)}`);
          break;
        }
      }
      if (pass) ok('quick');
    ...
    // T4: resetBoards count
    {
      const n = Global.size;
      resetBoards();
      const b = el.barsBubble.querySelectorAll('.bar').length;
      const q = el.barsQuick.querySelectorAll('.bar').length;
      if (b === n && q === n) {
        ok('resetBoards count');
      } else {
        ng('resetBoards count', `bubble=${b}, quick=${q}, n=${n}`);
      }
    }
    // T5: interval monotonic
    {
      Global.speed = 0.5;
      const i1 = computeInterval();
      Global.speed = 2.0;
      const i2 = computeInterval();
      if (i2 < i1) {
        ok('interval monotonic');
      } else {
        ng('interval monotonic', `i1=${i1}, i2=${i2}`);
      }
    }
    // T6: label swaps with values
    {
      const snap = snapshotBoard(Bubble);
      Bubble.data = [3, 1, 2];
      Bubble.ids = [1, 2, 3];
      renderBoard(Bubble, el.barsBubble, el.stepsBubble);
      applyStep(Bubble, el.barsBubble, { t: 'swap', i: 0, j: 1 });
      const l0 = el.barsBubble.querySelectorAll('.bar')[0].getAttribute('data-label');
      const l1 = el.barsBubble.querySelectorAll('.bar')[1].getAttribute('data-label');
      if (l0 === '2' && l1 === '1') {
        ok('label swap');
      } else {
        ng('label swap', `l0=${l0}, l1=${l1}`);
      }
      restoreBoard(Bubble, snap);
      renderBoard(Bubble, el.barsBubble, el.stepsBubble);
    }
    // T7: interval min bound
    {
      Global.speed = 999;
      if (computeInterval() >= MIN_TIMER_MS) {
        ok('interval min bound');
      } else {
        ng('interval min bound', `${computeInterval()}`);
      }
    }
    // T8: fixed transition speed
    {
      const before = getComputedStyle(document.documentElement)
        .getPropertyValue('--transMs')
        .trim();
      Global.speed = 0.2;
      rescheduleTimers();
      const after1 = getComputedStyle(document.documentElement)
        .getPropertyValue('--transMs')
        .trim();
      Global.speed = 10.0;
      rescheduleTimers();
      const after2 = getComputedStyle(document.documentElement)
        .getPropertyValue('--transMs')
        .trim();
      if (before === after1 && after1 === after2) {
        ok('fixed transition speed');
      } else {
        ng('fixed transition speed', `before=${before}, after1=${after1}, after2=${after2}`);
      }
    }
    // T9: details open by default
    {
      const d1 = document.getElementById('board-bubble') as HTMLDetailsElement;
      const d2 = document.getElementById('board-quick') as HTMLDetailsElement;
      if (d1.open && d2.open) {
        ok('details open by default');
      } else {
        ng('details open by default', `bubble=${d1.open}, quick=${d2.open}`);
      }
    }
    // T10: speed restored after tests
    {
      Global.speed = globalSnap.speed;
      el.speed.value = String(globalSnap.slider);
      el.speedVal.textContent = String(globalSnap.label);
      const after = computeInterval();
      if (
        parseFloat(el.speed.value) === globalSnap.speed &&
        el.speedVal.textContent === globalSnap.label &&
        after === computeInterval()
      ) {
        ok('speed restored after tests');
      } else {
        ng('speed restored after tests', `speed=${Global.speed}, label=${el.speedVal.textContent}`);
      }
    }
    // T11: quick has range/boundary
    {
      const st = buildQuickSteps([3, 2, 1]);
      const hasRange = st.some((s) => s.t === 'range');
      const hasBoundary = st.some((s) => s.t === 'boundary');
      if (hasRange && hasBoundary) {
        ok('quick has range/boundary');
      } else {
        ng('quick has range/boundary', JSON.stringify(st.slice(0, 6)));
      }
    }
    // T12: overlay style updated
    {
      updateQuickOverlay({ lo: 1, hi: 3, k: 2 });
      const bLeft = el.boundary.style.left;
      if (bLeft && bLeft.endsWith('%')) {
        ok('overlay style updated');
      } else {
        ng('overlay style updated', `left=${bLeft}`);
      }
      updateQuickOverlay({ lo: null, hi: null, k: null });
    }
    // T13: quick compare two bars
    {
      ...
      Quick.ids = [1, 2, 3];
      renderBoard(Quick, el.barsQuick, el.stepsQuick);
      applyStep(Quick, el.barsQuick, { t: 'compare', i: 0, j: 2 });
      const bars = el.barsQuick.querySelectorAll('.bar');
      const ok1 = bars[0].classList.contains('compare');
      const ok2 = bars[2].classList.contains('compare');
      if (ok1 && ok2) {
        ok('quick compare two bars');
      } else {
        ng('quick compare two bars', `${ok1},${ok2}`);
      }
      restoreBoard(Quick, snap);
      renderBoard(Quick, el.barsQuick, el.stepsQuick);
    }
    // T14: subrange visible
    {
      updateQuickOverlay({ lo: 0, hi: 2, k: 1 });
      const disp = el.subrange.style.display;
      if (disp === 'block') {
        ok('subrange visible');
      } else {
        ng('subrange visible', disp);
      }
      updateQuickOverlay({ lo: null, hi: null, k: null });
    }
    // T15: pivot line visible
    {
      ...
      Quick.ids = [1, 2, 3, 4];
      renderBoard(Quick, el.barsQuick, el.stepsQuick);
      Quick.range = { lo: 0, hi: 3 };
      applyStep(Quick, el.barsQuick, { t: 'pivot', i: 3 });
      updateQuickOverlay({ lo: 0, hi: 3, k: 0 });
      const disp = el.pivotLine.style.display;
      if (disp === 'block') {
        ok('pivot line visible');
      } else {
        ng('pivot line visible', disp || '');
      }
      restoreBoard(Quick, snap);
      renderBoard(Quick, el.barsQuick, el.stepsQuick);
      updateQuickOverlay({ lo: null, hi: null, k: null });
    ...
    // T16: quick swap indices in-bounds
    {
      ...
      const st = buildQuickSteps(c);
      const bad = st.filter(
        (s) => s.t === 'swap' && (s.i < 0 || s.j < 0 || s.i >= c.length || s.j >= c.length),
      );
      if (!bad.length) {
        ok('quick swap indices in-bounds');
      } else {
        ng('quick swap indices in-bounds', JSON.stringify(bad.slice(0, 3)));
      }
    }
    // T17: quick edge sorted/desc
    {
      ...
      for (const c of [
        ...
        if (!isSortedAsc(res)) {
          pass = false;
          ng('quick edge', `${JSON.stringify(c)} -> ${JSON.stringify(res)}`);
          break;
        }
      }
      if (pass) ok('quick edge');
    }
    // T18: genArray slider limits
    {
      const a = genArray(5);
      const b = genArray(50);
      if (a.length === 5 && b.length === 50) {
        ok('genArray slider limits');
      } else {
        ng('genArray slider limits', `${a.length},${b.length}`);
      }
    }
    // T19: candidate marks present
    {
      const st = buildQuickSteps([2, 5, 1, 4, 3]);
      const hasL = st.some((s) => s.t === 'markL');
      const hasR = st.some((s) => s.t === 'markR');
      if (hasL && hasR) {
        ok('candidate marks present');
      } else {
        ng(
          'candidate marks present',
          JSON.stringify(st.filter((s) => s.t === 'markL' || s.t === 'markR')),
        );
      }
    }
    // T20: boundary hidden in left scan
    {
      const st = buildQuickSteps([2, 3, 1]);
      const hidden = st.some((s) => s.t === 'boundary' && s.show === false);
      if (hidden) {
        ok('boundary hidden in left scan');
      } else {
        ng('boundary hidden in left scan', JSON.stringify(st.slice(0, 10)));
      }
    }
    // T21: boundary hidden after markL
    {
      const st = buildQuickSteps([1, 3, 2, 4]);
      const idxL = st.findIndex((s) => s.t === 'markL');
      let okFlag = false;
      if (idxL >= 0) {
        for (let u = idxL + 1; u < st.length; u++) {
          if (st[u].t === 'boundary' && st[u].show === false) {
            okFlag = true;
            break;
          }
          if (st[u].t === 'markR' || st[u].t === 'swap') break;
        }
      }
      if (okFlag) {
        ok('boundary hidden after markL');
      } else {
        ng('boundary hidden after markL');
      }
    }
    ...
    try {
      ...
      renderBoard(Quick, el.barsQuick, el.stepsQuick);
      Quick.range = { lo: 0, hi: 3 };
      Quick.boundaryIndex = 1;
      updateQuickOverlay({ lo: 0, hi: 3, k: 1, show: false });
      applyStep(Quick, el.barsQuick, { t: 'swap', i: 0, j: 2 });
      const dispAfterSwap = el.boundary.style.display;
      if (dispAfterSwap !== 'block') {
        ok('boundary hidden after swap');
      } else {
        ng('boundary hidden after swap', dispAfterSwap || '');
      }
      restoreBoard(Quick, snap);
      renderBoard(Quick, el.barsQuick, el.stepsQuick);
      updateQuickOverlay({ lo: null, hi: null, k: null });
    } catch (e) {
      ng('boundary hidden after swap (exception)', String(e));
    }
    // T23: boundary resets to lo on new range (extra test)
    try {
      const n = 10;
      Quick.data = Array.from({ length: n }, (_, i) => i + 1);
      Quick.ids = Array.from({ length: n }, (_, i) => i + 1);
      renderBoard(Quick, el.barsQuick, el.stepsQuick);
      Quick.range = null;
      Quick.boundaryIndex = 7;
      applyStep(Quick, el.barsQuick, { t: 'range', lo: 2, hi: 6 });
      const pass = Quick.boundaryIndex === 2;
      if (pass) {
        ok('boundary resets to lo on new range');
      } else {
        ng('boundary resets to lo on new range', Quick.boundaryIndex);
      }
      updateQuickOverlay({ lo: null, hi: null, k: null });
    } catch (e) {
      ng('boundary resets to lo on new range (exception)', String(e));
    }
    // T28: speed slider max 10x
    {
      const mx = parseFloat(el.speed.max);
      if (mx >= 10) {
        ok('speed slider max 10x');
      } else {
        ng('speed slider max 10x', `max=${mx}`);
      }
    }
    // T29: size slider max 50
    {
      const mx = parseInt(el.size.max, 10);
      if (mx >= 50) {
        ok('size slider max 50');
      } else {
        ng('size slider max 50', `max=${mx}`);
      }
    }
    ...
    try {
      ...
      el.size.value = '50';
      el.size.dispatchEvent(new Event('input', { bubbles: true }));
      const barsNow = el.barsBubble.querySelectorAll('.bar').length;
      const labelNow = (el.sizeVal.textContent || '').trim();
      if (barsNow === 50 && labelNow === '50') {
        ok('size slider reaches 50');
      } else {
        ng('size slider reaches 50', `bars=${barsNow}, label=${labelNow}`);
      }
      el.size.value = String(prev);
      el.size.dispatchEvent(new Event('input', { bubbles: true }));
    } catch (e) {
      ng('size slider reaches 50 (exception)', String(e));
    }
    // T31: linear 5x vs 2.5x
    {
      const eps = 1;
      const keep = Global.speed;
      Global.speed = 2.5;
      const i25 = computeInterval();
      Global.speed = 5.0;
      const i5 = computeInterval();
      Global.speed = keep;
      if (Math.abs(i5 * 2 - i25) <= eps) {
        ok('linearity 5x vs 2.5x');
      } else {
        ng('linearity 5x vs 2.5x', `i5=${i5}, i25=${i25}`);
      }
    }
    // T32: linear 2x vs 1x
    {
      ...
      const i2 = computeInterval();
      Global.speed = keep;
      if (Math.abs(i2 * 2 - i1) <= eps) {
        ok('linearity 2x vs 1x');
      } else {
        ng('linearity 2x vs 1x', `i1=${i1}, i2=${i2}`);
      }
    }
    // T33: linear 3x vs 1.5x
    {
      ...
      const keep = Global.speed;
      Global.speed = 1.5;
      const i15 = computeInterval();
      Global.speed = 3.0;
      const i3 = computeInterval();
      Global.speed = keep;
      if (Math.abs(i3 * 2 - i15) <= eps) {
        ok('linearity 3x vs 1.5x');
      } else {
        ng('linearity 3x vs 1.5x', `i15=${i15}, i3=${i3}`);
      }
    }
    // T34: linear 3x vs 1x
    {
      ...
      const i3 = computeInterval();
      Global.speed = keep;
      if (Math.abs(i3 * 3 - i1) <= eps) {
        ok('linearity 3x vs 1x');
      } else {
        ng('linearity 3x vs 1x', `i1=${i1}, i3=${i3}`);
      }
    }
    // T35: linear 10x vs 5x
    {
      ...
      const i5 = computeInterval();
      Global.speed = 10.0;
      const i10 = computeInterval();
      Global.speed = keep;
      if (Math.abs(i10 * 2 - i5) <= eps) {
        ok('linearity 10x vs 5x');
      } else {
        ng('linearity 10x vs 5x', `i10=${i10}, i5=${i5}`);
      }
    }
  }
  ...
  el.speed.addEventListener('input', (e) => {
    ...
    Global.speed = sp;
    el.speedVal.textContent = sp.toFixed(2) + 'x';
    if (Global.playing) rescheduleTimers();
    else updateVisualSpeed();
  });
  // steppers for speed
